   ...
   for(unsigned int l=0; l<pfCluster->size(); l++) // Iterating over sim clusters
   {
     ...
     { 
       ...
       for(unsigned ifrac = 0; ifrac < pfCluster->at(l).hitsAndFractions().size(); ifrac++)
       {
         ...
         {
           DetId detId_recHit = hgcHitEE->detid();
           if(id==detId_recHit.rawId() and hgcHitEE->time() > 0)
           {
             const GlobalPoint& recHitPos = geoHandle->getPosition(id); 
             recHit_x.push_back(recHitPos.x());
             recHit_y.push_back(recHitPos.y());
             recHit_z.push_back(recHitPos.z());
