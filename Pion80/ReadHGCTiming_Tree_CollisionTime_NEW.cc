...
int ReadHGCTiming_Tree_Jun3(std::string infile, std::string outfile, double energyLow, double energyHigh)
{
  ...
  for (int i=0; i<nEvents; ++i)
  {
    ...
    for (int j=0; j<=hitNumber; j++)
    {
      ...
      sumOfHitEnergyAllWindow += recHits.at(j).recHitE;
      sumOfCollTime += recHits.at(j).recHitTime + gRandom->Gaus(0, 0.05);
      if(recHits.at(j).recHitTime < 0.01) sumOfHitEnergyTimesAllWindowCollTime += (recHits.at(j).recHitTime + gRandom->Gaus(0, 0.05))*recHits.at(j).recHitE;
    ...
    if(sumOfHitEnergyAllWindow > 0.0) h_TimeAverage_collTime_energyWeighted->Fill(sumOfHitEnergyTimesAllWindowCollTime/sumOfHitEnergyAllWindow); 
